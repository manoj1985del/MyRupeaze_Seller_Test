...
if (type == "all") {
     query = firebase.firestore().collection("doctor");
}

if (type == "byseller") {
    var merchantid = getQueryVariable("merchant_id");
    query = firebase.firestore().collection("doctor").where("merchant_id", "==", merchantid);
}

if (type == "pending") {
    query = firebase.firestore().collection("doctor").where("status", "==", "pending");
}

if (type == "suspended") {
    query = firebase.firestore().collection("doctor").where("status", "==", "suspended");
}

if (type == "approved") {
    query = firebase.firestore().collection("doctor").where("status", "==", "approved");
}
...
function createTableHeaders() {
    ...
    var doctorImg = document.createElement("th");
    ...
    doctorImg.innerHTML = "Doctor Imgage";
...
function drawTable(sellerList) {
    ...
    for (var i = 0; i < sellerList.length; i++) {
        ...
        var tdDoctorImg = document.createElement("td");
        ...
        var divDoctorImg = document.createElement("div");
        var imgDoctor = document.createElement("img");
        imgDoctor.setAttribute("id", i.toString());
        imgDoctor.setAttribute("src", seller.doctor_img_url);
        divDoctorImg.appendChild(imgDoctor);
        ...
        var divDownloadDegree = document.createElement("div");
        var btnDownloadDegree = document.createElement("button");
        btnDownloadDegree.textContent = "Download Degree";
        btnDownloadDegree.style.width = "150px";
        btnDownloadDegree.setAttribute("id", i.toString());
        btnDownloadDegree.style.marginBottom = "10px";
        divDownloadDegree.appendChild(btnDownloadDegree);
        divApprove.appendChild(divDownloadDegree);
        ...
        tdDoctorImg.appendChild(divDoctorImg);
        ...
        btnReject.addEventListener("click", function () {
            ...
            getSellerProducts(seller).then(() => {
                ...
                Promise.all(promiseList).then(() => {
                    ...
                    var washingtonRef = firebase.firestore().collection("doctor").doc(seller.seller_id);
                    washingtonRef.update({
                        status: "rejected",
                        suspension_reason: reason
                    })
                        .then(function () {
                            ...
                            window.location.href = "admin_doctor_listing.html?type=all";
        ...
        //Download Degree Button Click

        btnDownloadDegree.addEventListener("click", function(){

            var index = parseInt(this.id);
            var seller = sellerList[index];
            console.log("index = " + index + " " + seller.degree_url);
