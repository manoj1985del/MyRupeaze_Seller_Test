...
if (type == "all") {
     query = firebase.firestore().collection("pharmacist");
}

if (type == "byseller") {
    var merchantid = getQueryVariable("merchant_id");
    query = firebase.firestore().collection("pharmacist").where("merchant_id", "==", merchantid);
}

if (type == "pending") {
    query = firebase.firestore().collection("pharmacist").where("status", "==", "pending");
}

if (type == "suspended") {
    query = firebase.firestore().collection("pharmacist").where("status", "==", "suspended");
}

if (type == "approved") {
    query = firebase.firestore().collection("pharmacist").where("status", "==", "approved");
}
...
function createTableHeaders() {
    ...
    sellerDetailsHeader.innerHTML = "Pharmacist Details";
        ...
        btnReject.addEventListener("click", function () {
            ...
            getSellerProducts(seller).then(() => {
                ...
                Promise.all(promiseList).then(() => {
                    ...
                    var washingtonRef = firebase.firestore().collection("pharmacist").doc(seller.seller_id);
                    washingtonRef.update({
                        status: "rejected",
                        suspension_reason: reason
                    })
                        .then(function () {
                            ...
                            window.location.href = "admin_pharmacist_listing.html?type=all";
